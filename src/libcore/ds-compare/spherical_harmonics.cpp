...
Sample SphericalHarmonics::sample(Point2& pos)
{
    ...
    if (pos.y == 0)
    {
        sample.pdf = Epsilon;
        sample.theta = 0;
        sample.phi = 2 * M_PI * pos.x;
        return sample;
    }

    sample.pdf = std::max(this->sampler->warp(*this, pos), Epsilon);
    sample.theta = pos.x;
    sample.phi = pos.y;
    return sample;
}
...
bool SphericalHarmonics::isAzimuthallyInvariant() const {
...
void SphericalHarmonics::addOffset(Float value) {
    operator()(0, 0) += 2 * value * (Float) std::sqrt(M_PI);
}

Float SphericalHarmonics::eval(const Vector &v) const {
    Float result = 0;
...
Float SphericalHarmonics::evalAzimuthallyInvariant(Float theta, Float phi) const {
...
Float SphericalHarmonics::evalAzimuthallyInvariant(const Vector &v) const {
...
Matrix3x3 SphericalHarmonics::mu2() const {
...
std::string SphericalHarmonics::toString() const {
    ...
    oss << "SphericalHarmonics[bands=" << m_bands << ", {";
    int pos = 0;
...
Float SphericalHarmonics::computeNormalization(int l, int m) {
    SAssert(m>=0);
    return std::sqrt(
...
void SphericalHarmonics::staticInitialization() {
...
void SphericalHarmonics::staticShutdown() {
    delete[] m_normalization;
    m_normalization = NULL;
}

struct RotationBlockHelper {
    const SphericalHarmonicsRotation::Matrix &M1, &Mp;
    SphericalHarmonicsRotation::Matrix &Mn;
    int prevLevel, level;

    inline RotationBlockHelper(
        const SphericalHarmonicsRotation::Matrix &M1,
        const SphericalHarmonicsRotation::Matrix &Mp,
        SphericalHarmonicsRotation::Matrix &Mn)
...
void SphericalHarmonics::rotationBlock(
        const SphericalHarmonicsRotation::Matrix &M1,
        const SphericalHarmonicsRotation::Matrix &Mp,
        SphericalHarmonicsRotation::Matrix &Mn) {
...
void SphericalHarmonics::rotation(const Transform &t, SphericalHarmonicsRotation &rot) {
    rot.blocks[0](0, 0) = 1;
...
void SphericalHarmonicsRotation::operator()(const SphericalHarmonics &source, SphericalHarmonics &target) const {
    SAssert(source.getBands() == target.getBands());
    for (int l=0; l<source.getBands(); ++l) {
        const SphericalHarmonicsRotation::Matrix &M = blocks[l];
...
SphericalHarmonicsSampler::SphericalHarmonicsSampler(int bands, int depth) : m_bands(bands), m_depth(depth) {
...
std::string SphericalHarmonicsSampler::toString() const {
    std::ostringstream oss;
...
Float SphericalHarmonicsSampler::integrate(int depth, int zBlock, int phiBlock, const SphericalHarmonics &f) const {
    Float result = 0;
...
Float *SphericalHarmonicsSampler::phiIntegrals(Float a, Float b) {
...
Float *SphericalHarmonicsSampler::legendreIntegrals(Float a, Float b) {
...
MTS_IMPLEMENT_CLASS(SphericalHarmonicsSampler, false, Object)
MTS_NAMESPACE_END
