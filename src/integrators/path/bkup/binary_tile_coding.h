...
struct BTTracker {
    ...
    inline void set_boundaries(const Pair<float> x, const Pair<float> y)
    {
        this->x_bounds = x;
        this->y_bounds = y;
    }
    ...
    {
        Pair<float> bounds = (dir == Horizontal) ? this->x_bounds : this->y_bounds;
        return Pair<float>(
...
struct LeafNode {
    ...
    Pair<float> cov = Pair<float>(0.0f);
    // Sample mean required for covariance updates.
    Pair<float> sample_mean = Pair<float>(0.0f);
...
struct BinaryTiling {
    ...
    BinaryTile& find_tile(const Pair<float>& pos, BTTracker& counter, bool find_empty = true);
    ...
    inline std::pair<Pair<float>, Pair<float>> base_tile_bounds(int x, int y) const;

    /// Utility function to obtain the 2D index of a base tile.
    inline Pair<int> base_tile_pos(const Pair<float>& pos);
...
struct BinaryTileCoding {
    ...
    Sample sample(Pair<float>& pos, std::function<float()> rng);
