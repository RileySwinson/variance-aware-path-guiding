...
BinaryTile& BinaryTiling::find_tile(const Pair<float>& pos, BTTracker& tracker, bool find_empty)
{
    ...
    Pair<int> bt_pos(
        std::max(0, std::min((int)(x_warped * td.x), td.x - 1)),
        std::max(0, std::min((int)(y_warped * td.y), td.y - 1))
    );
...
void BinaryTiling::insert(const Sample& sample, uint32_t& split_count)
{
    ...
    if (below_max_depth && below_max_splits && tile.should_split())
    {
        ...
        // added area-weighted deposition of child radiance
        float parentSum = tile.sum;
        ...
        child0.sum = parentSum * 0.5f;
        child1.sum = parentSum * 0.5f;
...
float BinaryTiling::recurse_statistics(BinaryTile& curr_tile, BTTracker tracker, float& leaf_sum)
{
    ...
    {
        // old code
        // curr_tile.sum * curr_tile.area(tracker, true)
        // leaf_sum += power;
...
size_t BinaryTileCoding::nodeCount() const {
    size_t count = 0;
    
    for (const auto& tiling : this->tilings) {
        count += tiling.tiles.size();
    }
...
Sample BinaryTileCoding::sample(Pair<float>& pos, std::function<float()> rng)
{
    if (this->leaf_sum <= 0.0f)
    {
        ...
        Sample empty;
        empty.phi = 2.0f * M_PI * rx;
        empty.theta = std::acos(1.0f - 2.0f * ry);
        empty.value = 0;
        empty.p = INV_FOURPI * std::sin(ry);
