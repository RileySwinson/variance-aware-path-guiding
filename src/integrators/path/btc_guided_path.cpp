/*
    ...
    Copyright (c) 2020 by Alexander Rath.
...
// all additions for our paper "Variance-aware path guiding"
// have been marked with "@addition" so they can easily be found using search
...
public:
    ...
    void step(float gradient) {
        ...
        // Clamp to [-20, 20] since sigmoid makes these near-zzero
...
public:
    BTCAdapter() :
        m_buildMutex(std::make_unique<std::mutex>())
    {}

    BTCAdapter(const BTCAdapter& other) :
        ...
        m_statisticalWeightBuilding(other.m_statisticalWeightBuilding),
        m_bsdfSamplingFractionOptimizer(other.m_bsdfSamplingFractionOptimizer),
        m_buildMutex(std::make_unique<std::mutex>())
    {}

    BTCAdapter& operator=(const BTCAdapter& other) {
        if (this != &other) {
            m_building = other.m_building;
            m_sampling = other.m_sampling;
            m_isBuilt = other.m_isBuilt;
    ...
    BTCAdapter(BTCAdapter&& other) noexcept :
        m_building(std::move(other.m_building)),
        m_sampling(std::move(other.m_sampling)),
        m_isBuilt(other.m_isBuilt),
        m_statisticalWeight(other.m_statisticalWeight),
        m_statisticalWeightBuilding(other.m_statisticalWeightBuilding),
        m_bsdfSamplingFractionOptimizer(other.m_bsdfSamplingFractionOptimizer),
    ...
    BTCAdapter& operator=(BTCAdapter&& other) noexcept {
        if (this != &other) {
            m_building = std::move(other.m_building);
            m_sampling = std::move(other.m_sampling);
            m_isBuilt = other.m_isBuilt;
    ...
    // tiling count, base tiles in both dimensions, tiling domain excess, max splits, eagerness
    static TC::BTCArguments s_btcArgs;

    void record(
        const DTreeRecord& rec, EDirectionalFilter directionalFilter,
        EBsdfSamplingFractionLoss bsdfSamplingFractionLoss
    ) {
        // Match vanilla guided_path semantics exactly so spatial-tree remfine()
        // sees the same statisticalWeightBuilding as Mueller's reference.

        const bool statValid = std::isfinite(rec.statisticalWeight) && rec.statisticalWeight > 0.0f;

        // Optimizer gradient is computed independently of isDelta / stat weight,
        // gated only on (loss enabled && rec.product > 0), like vanilla.
        float convergenceGradient = 0.0f;
        bool hasGradient = false;
        if (bsdfSamplingFractionLoss != EBsdfSamplingFractionLoss::ENone && rec.product > 0) {
            if (bsdfSamplingFractionLoss == EBsdfSamplingFractionLoss::EKL) {
                convergenceGradient = rec.product / rec.woPdf * (rec.bsdfPdf / rec.woPdf - 1.0f);
            } else if (bsdfSamplingFractionLoss == EBsdfSamplingFractionLoss::EVariance) {
                float convergenceEstimate = rec.product / rec.woPdf;
                convergenceGradient = convergenceEstimate * convergenceEstimate * (rec.bsdfPdf - rec.dTreePdf) / (rec.woPdf * rec.woPdf);
            }
            hasGradient = std::isfinite(convergenceGradient);
        }

        std::lock_guard<std::mutex> lock(*m_buildMutex);

        // Vanilla's recordIrradiance: bump stat weight on every non-delta path
        // with a valid stat weight, regardless of radiance/irradiance validity.
        // Tree storage is gated separately on isfinite(irradiance) && irradiance > 0.
        if (!rec.isDelta && statValid) {
            m_statisticalWeightBuilding += rec.statisticalWeight;

            float irradiance = rec.radiance / rec.woPdf;
            if (std::isfinite(irradiance) && irradiance > 0.0f) {
                Point2 angles = dirToSpherical(rec.d);
                TC::Sample s;
                s.phi = angles.y;
                s.theta = angles.x;
                s.value = irradiance;
                s.pdf = 1.0f / (4.0f * M_PI);
                m_building.store(s);
            }
        }

        if (hasGradient) {
            m_bsdfSamplingFractionOptimizer.append(convergenceGradient, rec.statisticalWeight);
        }
    }

    void build(EDistribution distribution) {
        m_building.postprocess();
        m_sampling = m_building;
        m_statisticalWeight = m_statisticalWeightBuilding;
        m_isBuilt = true;

        // EDistribution is ignored; VAPG is not currently here
    }

    // passing parameters to reset is kinda redundant with BTC
    void reset(int maxDepth, float subdivisionThreshold) {
        m_building.wipe();
        m_building.construct(s_btcArgs);
        m_building.preprocess();
        m_statisticalWeightBuilding = 0.0f;
    }

    // I gave up trying to fully trace and understand where the Sampler is selected/defined and just accepted this works
    // I note BTC uses its own sampler anyways
    Vector sample(Sampler* sampler) const {
        // fallback to uniform sphere sample with pdf 1/4pi
        if (!m_isBuilt || m_sampling.leaf_sum <= 0.0f) {
            float z = 1.0f - 2.0f * sampler->next1D();
            float r = std::sqrt(std::max(0.0f, 1.0f - z * z));
            float phi = 2.0f * M_PI * sampler->next1D();
            return Vector(r * std::cos(phi), r * std::sin(phi), z);
        }

        TC::Pair<float> pos(sampler->next1D(), sampler->next1D());
        TC::Sample result = const_cast<TC::BinaryTileCoding&>(m_sampling).sample(pos);

        float theta = result.theta; // polar angle stays polar
        float phi = result.phi; // azimuthal stays azimuthal
    ...
    float pdf(const Vector& dir) const {
        ...
        Point2 angles = dirToSpherical(dir);
        float theta = angles.x;
        float phi = angles.y;

        TC::Pair<float> uv = TC::Util::spherical_to_uv(
            TC::Pair<float>(phi, theta)
        );

        float density = const_cast<TC::BinaryTileCoding&>(m_sampling).eval(uv);
        // note i used to have a jacobian correction 2 pi^2 sin(theta) here; was not needed bc eval() isn't uv-space
        return std::max(density, 1e-10f);
    }

    void read(BlobReader& blob) {}
    void dump(BlobWriter& blob) const {
        const auto& tilings = m_sampling.getTilings();
        uint32_t numTilings = (uint32_t)tilings.size();
        blob << numTilings;
        blob << m_sampling.leaf_sum;

        for (size_t ti = 0; ti < tilings.size(); ++ti) {
            const auto& tiling = tilings[ti];

            blob << tiling.x_bounds.x << tiling.x_bounds.y;
            // flip the polar axis in the dump to match muller's dump 
            blob << (1.0f - tiling.y_bounds.y) << (1.0f - tiling.y_bounds.x);
            blob << tiling.total_power;

            uint32_t bx = (uint32_t)TC::BinaryTileCoding::TILE_DIMS.x;
            uint32_t by = (uint32_t)TC::BinaryTileCoding::TILE_DIMS.y;
            blob << bx << by;

            uint32_t numTiles = (uint32_t)tiling.tiles.size();
            blob << numTiles;

            for (size_t j = 0; j < tiling.tiles.size(); ++j) {
                const auto& tile = tiling.tiles[j];

                blob << tile.sum;
                uint32_t sampleCount = (uint32_t)tile.data.sample_count;
                blob << sampleCount;

                {
                    uint16_t sz = 1;
                    uint8_t val = (tile.data.tile_type == TC::TileType::Internal) ? 1 : 0;
                    blob.Write(&sz, 1);
                    blob.Write(&val, 1);
                }

                if (tile.data.tile_type == TC::TileType::Internal) {
                    // Vertical/polar splits swap the children so child[0] is the lower-v half
                    // after mirroring the vertical axis
                    const bool flipY = (tile.data.split_direction == TC::Direction::Vertical);
                    blob << tile.internal.power[flipY ? 1 : 0];
                    blob << tile.internal.power[flipY ? 0 : 1];
                    blob << tile.internal.children[flipY ? 1 : 0];
                    blob << tile.internal.children[flipY ? 0 : 1];

                    uint16_t sz = 1;
                    uint8_t val = (tile.data.split_direction == TC::Direction::Horizontal) ? 0 : 1;
                    blob.Write(&sz, 1);
                    blob.Write(&val, 1);
                } else {
                    blob << tile.leaf.cov.x;
                    blob << -tile.leaf.cov.y; // mirrored polar axis flips the sign of the y covariance
                    blob << tile.leaf.sample_mean.x;
                    blob << ((float) M_PI - tile.leaf.sample_mean.y); // sample_mean.y is theta in radians
                    blob << tile.leaf.m2;
                    blob << tile.leaf.ad_sum;
                }
            }
        }
    }

    void setStatisticalWeightBuilding(float w) { m_statisticalWeightBuilding = w; }
    float statisticalWeightBuilding() const { return m_statisticalWeightBuilding; }

    size_t approxMemoryFootprint() const {
        return sizeof(*this)
            + const_cast<TC::BinaryTileCoding&>(m_building).memory()
            + const_cast<TC::BinaryTileCoding&>(m_sampling).memory();
    }

    int depth() const { return m_isBuilt ? 1 : 0; }

    size_t numNodes() const {
        return m_isBuilt ? m_sampling.nodeCount() : 0;
    }

    float statisticalWeight() const { return m_statisticalWeight; }

    float meanRadiance() const {
        return m_isBuilt ? m_sampling.leaf_sum / (4.0f * M_PI) : 0.0f;
    }

    float bsdfSamplingFraction() const {
        return logistic(m_bsdfSamplingFractionOptimizer.variable());
    }

private:
    TC::BinaryTileCoding m_building;
    TC::BinaryTileCoding m_sampling;
    bool m_isBuilt = false;

    float m_statisticalWeight = 0.0f;
    float m_statisticalWeightBuilding = 0.0f;

    AdamOptimizer m_bsdfSamplingFractionOptimizer{0.01f};
    mutable std::unique_ptr<std::mutex> m_buildMutex;

    static Point2 dirToSpherical(const Vector& d) {
        // degenerative case
        if (!std::isfinite(d.x) || !std::isfinite(d.y) || !std::isfinite(d.z)) { 
            return {0, 0}; 
        }

        float theta = std::acos(std::min(std::max(d.z, -1.0f), 1.0f)); // guard for floating-point precision errors
        float phi = std::atan2(d.y, d.x); 

        // BTC operates in [0, 2pi)
        if (phi < 0) { phi += 2 * M_PI; } 
    ...
    static Point2 dirToCanonical(const Vector& d) {
        ...
        return {phi / (2 * M_PI), (cosTheta + 1) / 2};
    }
};

TC::BTCArguments BTCAdapter::s_btcArgs;
...
struct STreeNode {
    ...
    BTCAdapter* dTreeWrapper(Point& p, Vector& size, std::vector<STreeNode>& nodes) {
    ...
    const BTCAdapter* dTreeWrapper() const {
        return &btc;
    }
    ...
    void forEachLeaf(
        std::function<void(const BTCAdapter*, const Point&, const Vector&)> func,
    ...
    void record(const Point& min1, const Point& max1, Point min2, Vector size2, const DTreeRecord& rec, EDirectionalFilter directionalFilter, EBsdfSamplingFractionLoss bsdfSamplingFractionLoss, std::vector<STreeNode>& nodes) {
        ...
        if (w > 0) {
            if (isLeaf) {
                btc.record({ rec.d, rec.radiance, rec.product, rec.woPdf, rec.bsdfPdf, rec.dTreePdf, rec.statisticalWeight * w, rec.isDelta }, directionalFilter, bsdfSamplingFractionLoss);
            } else {
                size2[axis] /= 2;
                for (int i = 0; i < 2; ++i) {
                    if (i & 1) {
...
public:
    ...
    void subdivide(int nodeIdx, std::vector<STreeNode>& nodes) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            nodes[idx].btc = cur.btc;
            nodes[idx].btc.setStatisticalWeightBuilding(nodes[idx].btc.statisticalWeightBuilding() / 2);
        }
        cur.isLeaf = false;
        cur.btc = {}; // Reset to an empty dtree to save memory.
    }

    BTCAdapter* dTreeWrapper(Point p, Vector& size) {
    ...
    BTCAdapter* dTreeWrapper(Point p) {
        Vector size;
        return dTreeWrapper(p, size);
    }

    void forEachDTreeWrapperConst(std::function<void(const BTCAdapter*)> func) const {
    ...
    void forEachDTreeWrapperConstP(std::function<void(const BTCAdapter*, const Point&, const Vector&)> func) const {
        m_nodes[0].forEachLeaf(func, m_aabb.min, m_aabb.max - m_aabb.min, m_nodes);
    }

    void forEachDTreeWrapperParallel(std::function<void(BTCAdapter*)> func) {
...
#pragma omp parallel for
    ...
    bool shallSplit(const STreeNode& node, int depth, size_t samplesRequired) {
        return m_nodes.size() < std::numeric_limits<uint32_t>::max() - 1 && node.btc.statisticalWeightBuilding() > samplesRequired;
    }
...
public:
    GuidedPathTracer(const Properties &props) : MonteCarloIntegrator(props) {
        ...
        if (!Thread::getThread()->getLogger()->readLog(m_configLog))
            Log(EInfo, "could not retrieve log");
        
        m_neeStr = props.getString("nee", "always");
        if (m_neeStr == "never") {
            m_nee = ENever;
        ...
        m_sampleCombinationStr = props.getString("sampleCombination", "discard");
        ...
        m_spatialFilterStr = props.getString("spatialFilter", "box");
        ...
        m_directionalFilterStr = props.getString("directionalFilter", "box");
        ...
        m_distributionStr = props.getString("distribution", "radiance");
        if (m_distributionStr == "radiance") {
            m_distribution = EDistribution::ERadiance;
        } else if (m_distributionStr == "simple") {
            m_distribution = EDistribution::ESimple;
        } else if (m_distributionStr == "full") {
            m_distribution = EDistribution::EFull;
        } else {
            Assert(false);
        }

        auto diStr = props.getString("diStrategy", "no");
        if (diStr == "no"        ) m_diStrategy = EDIStrategy::ENoDI; else
        if (diStr == "weighted"  ) m_diStrategy = EDIStrategy::EWeightedDI; else
        if (diStr == "unweighted") m_diStrategy = EDIStrategy::EUnweightedDI; else
        Assert(false);
        ...
        m_sTreeThreshold = props.getInteger("sTreeThreshold", 2000);
        if (props.hasProperty("maxSplits"))
            BTCAdapter::s_btcArgs.max_splits = props.getInteger("maxSplits"); // maximum number of splits, default infinity
        if (props.hasProperty("eagerness"))
            BTCAdapter::s_btcArgs.eagerness = props.getInteger("eagerness"); // eagerness to split, default 0
        m_dTreeThreshold = props.getFloat("dTreeThreshold", 0.005f);
        ...
        m_trainingIterations = props.getInteger("trainingIterations", -1);
    }
    ...
    void resetSDTree() {
        ...
        m_sdTree->forEachDTreeWrapperParallel([this](BTCAdapter* dTree) { dTree->reset(20, m_dTreeThreshold); }); // maxDepth lies here; muller passes 20
    }
    ...
    void buildSDTree(bool doBuild = true) {
        if (doBuild) {
            ...
            // Build distributions
            m_sdTree->forEachDTreeWrapperParallel([this](BTCAdapter* dTree) {
        ...
        float approxMemoryFootprint = 0;

        int nPoints = 0;
        int nPointsNodes = 0;

        // Structure-size telemetry (per iteration). Total directional tiles across all
        // spatial cells, plus how many cells saturated the per-structure split cap.
        size_t totalTiles = 0;
        int nSaturated = 0;
        const size_t satTiles =
            (size_t)BTCAdapter::s_btcArgs.tilings
            * (size_t)BTCAdapter::s_btcArgs.tiles.x
            * (size_t)BTCAdapter::s_btcArgs.tiles.y
            + 2 * (size_t)BTCAdapter::s_btcArgs.max_splits;

        m_sdTree->forEachDTreeWrapperConst([&](const BTCAdapter* dTree) {
            ...
            approxMemoryFootprint += dTree->approxMemoryFootprint();

            const size_t tiles = dTree->numNodes();
            totalTiles += tiles;
            if (tiles >= satTiles) ++nSaturated;
        ...
        Log(EInfo,
            ...
            "  Stat. weight  = [%f, %f, %f]\n"
            "  Spatial cells = %d\n"
            "  Total tiles   = " SIZE_T_FMT "\n"
            "  Saturated cells = %d / %d  (split cap = %d)\n"
            "  Aprox. Memory = %f bytes \n",
            ...
            nPoints,
            totalTiles,
            nSaturated, nPoints, BTCAdapter::s_btcArgs.max_splits,
    ...
    l_abort:
        ...
        // @addition calculate ad-hoc pixel estimate using iterated box filters
        // results can be slightly enhanced with a properly denoised pixel estimate,
        // which is, however, orthogonal to our work
        if (m_distribution == EDistribution::EFull && !m_isFinalIter) {
            Vector2i size = image->getSize();
            m_pixelEstimate = image->clone();
            
            int blackPixels = 0;
            auto data = static_cast<float *>(m_pixelEstimate->getData());
            int fpp = int(m_pixelEstimate->getBytesPerPixel() / sizeof(float)); // floats per pixel
            for (int i = 0, j = size.x*size.y; i < j; ++i) {
                int isBlack = true;
            ...
            float blackDensity = 1.f / (std::sqrt(1.f - blackPixels / float(size.x*size.y)) + 1e-3);
            
            const int FILTER_WIDTH = std::max(std::min(int(3 * blackDensity), std::min(size.x, size.y)-1), 1);
            const int FILTER_ITERATIONS = 3;
            ...
            for (int chan = 0; chan < SPECTRUM_SAMPLES; ++chan) {
                for (int iter = 0; iter < FILTER_ITERATIONS; ++iter) {
                    for (int x = 0; x < size.x; ++x)
                        boxFilter(data + chan + x*fpp, size.x*fpp, size.y);
                    for (int y = 0; y < size.y; ++y)
                        boxFilter(data + chan + y*size.x*fpp, fpp, size.x);
                }
                for (int i = 0, j = size.x * size.y; i < j; ++i) {
    ...
    bool renderTime(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        while (result && elapsedSeconds < nSeconds) {
            ...
            if (
                (elapsedSeconds / float(nSeconds) >= 0.15 && m_trainingIterations == -1)
                || m_trainingIterations == m_iter
            ) {
                Log(EInfo, "FINAL %f seconds", remainingTime);
                m_isFinalIter = true;

                float secsPerPass;
                int passes = passesThisIteration;
                
                do {
                    float elapsedBefore = elapsedSeconds;

                    if (!performRenderPasses(variance, passes, scene, queue, job, sceneResID, sensorResID, samplerResID, integratorResID)) {
                        result = false;
                        break;
                    }

                    elapsedSeconds = computeElapsedSeconds(m_startTime);
                    secsPerPass = (elapsedSeconds - elapsedBefore) / passes;

                    passes = std::max(1, std::min(128, int((nSeconds - elapsedSeconds) / secsPerPass)));
    ...
    void renderBlock(const Scene *scene, const Sensor *sensor,
        ...
        for (size_t i = 0; i < points.size(); ++i) {
            ...
            // @addition
            // note that the pixel estimate is only needed for our full distribution
            Spectrum pixelEstimate;
            if (m_pixelEstimate.get()) {
                pixelEstimate = m_pixelEstimate->getPixel(offset);
            } else {
                // if this is the first iteration, we assume that the image is uniformly 50% gray
                pixelEstimate = Spectrum { 0.5f };
            }
            ...
            for (int j = 0; j < m_sppPerPass; j++) {
                ...
                Spectrum spec = sensor->sampleRay(
                    sensorRay, samplePos, apertureSample, timeSample);

                spec = Li(sensorRay, rRec, spec, pixelEstimate);
                //if (m_isFinalIter) spec = pixelEstimate;
    ...
    Spectrum sampleMat(const BSDF* bsdf, BSDFSamplingRecord& bRec, float& woPdf, float& bsdfPdf, float& dTreePdf, float bsdfSamplingFraction, RadianceQueryRecord& rRec, const BTCAdapter* dTree) const {
    ...
    void pdfMat(float& woPdf, float& bsdfPdf, float& dTreePdf, float bsdfSamplingFraction, const BSDF* bsdf, const BSDFSamplingRecord& bRec, const BTCAdapter* dTree) const {
        dTreePdf = 0;
    ...
    Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        NotImplementedError("Pixel estimate and throughput must be provided for full distribution");
    }
    ...
    Spectrum Li(
        ...
        struct Vertex {
            ...
            void commit(STree& sdTree, float statisticalWeight, ESpatialFilter spatialFilter, EDirectionalFilter directionalFilter, EBsdfSamplingFractionLoss bsdfSamplingFractionLoss, EDistribution distribution, Sampler* sampler, const Spectrum &pixelEstimate) {
                ...
                switch (distribution) {
                    case EDistribution::ERadiance:
                        value = localRadiance.average();
                        break;
                    
                    case EDistribution::ESimple:
                        value = (bsdfVal * localRadiance).average(); // partial integrand
                        if (woMisWeight > 0)
                            // mis compensation
                            value *= woMisWeight;
                        value = std::pow(value, 2); // second moment
                        break;
                    ...
                    case EDistribution::EFull:
                        ...
                        value = (radiance / pixelEstimate * woPdf).average(); // full integrand
                        if (woMisWeight > 0)
                            // mis compensation
                            value *= woMisWeight;
                        value = std::pow(value, 2); // second moment
                ...
                switch (distribution) {
                    case EDistribution::ERadiance:
                        // previous selection probability loss (Müller et al.)
                        lossValue = (localRadiance * bsdfVal).average();
                        break;
                    
                    case EDistribution::ESimple:
                    case EDistribution::EFull:
                        // our selection probability loss
                        // note that since 'radiance = throughput * localRadiance',
                        // we do not need a multiplication by throughput here.
                        lossValue = (radiance / pixelEstimate * woPdf).average(); // full integrand
                        break;
                ...
                DTreeRecord rec{ ray.d, value, lossValue, woPdf, bsdfPdf, dTreePdf, statisticalWeight, isDelta };
                ...
                if (m_doNee &&
                    ...
                    if (!value.isZero()) {
                        ...
                        if (!m_strictNormals || woDotGeoN * Frame::cosTheta(bRec.wo) > 0) {
                            ...
                            if (!m_isFinalIter && m_diStrategy != EDIStrategy::ENoDI) {
                                if (dTree) {
                                    Vertex v = Vertex{
                                        dTree,
                                        dTreeVoxelSize,
                                        ...
                                        bsdfVal,
                                        m_diStrategy == EDIStrategy::EUnweightedDI ?
                                            L :
                                            L * miWeight(woPdf, dRec.pdf),
                                        dRec.pdf,
                                        bsdfPdf,
                                        dTreePdf,
                                        dTreePdf / woPdf,
                                    ...
                                    v.commit(*m_sdTree, 1.0f, m_spatialFilter, m_directionalFilter, m_isBuilt ? m_bsdfSamplingFractionLoss : EBsdfSamplingFractionLoss::ENone, m_distribution, rRec.sampler, pixelEstimate);
                                }
                            }
                ...
                if (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance) {
                    ...
                    if ((!isDelta || m_bsdfSamplingFractionLoss != EBsdfSamplingFractionLoss::ENone) && dTree && nVertices < MAX_NUM_VERTICES && !m_isFinalIter) {
                        if (1 / woPdf > 0) {
                            ...
                            if (m_doNee) {
                                switch (m_diStrategy) {
                                    case EDIStrategy::ENoDI:
                                        Ldi = Spectrum { 0.f };
                                        break;
                                    case EDIStrategy::EWeightedDI:
                                        Ldi = L * miWeight(woPdf, emitterPdf);
                                        break;
                                    case EDIStrategy::EUnweightedDI:
                                        Ldi = L;
                                        break;
                                    default:
                                        Assert(false);
        ...
        if (nVertices > 0 && !m_isFinalIter) {
            for (int i = 0; i < nVertices; ++i) {
                vertices[i].commit(*m_sdTree, 1.0f, m_spatialFilter, m_directionalFilter, m_isBuilt ? m_bsdfSamplingFractionLoss : EBsdfSamplingFractionLoss::ENone, m_distribution, rRec.sampler, pixelEstimate);
            }
        }
...
private:
    ...
    */
    ...
    /**
        ...
        The following values are valid:
        - "radiance": Previous guiding distribution [Mueller et al. 2017].
        - "simple":   Approximates parts of the integrand
                      Robust for short renders, but worse than "full" for long renders
        - "full":     Approximates the full integrand
                      Recommended for very long renders, otherwise use "simple"
        Default     = "radiance" (for reproducibility)
        Recommended = "simple" for short renders
    */
    std::string m_distributionStr;
    EDistribution m_distribution;

    EDIStrategy m_diStrategy;
