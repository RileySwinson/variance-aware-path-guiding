...
public:
    ...
    int BTCAdapter::s_maxSplits = std::numeric_limits<int>::max;
    int BTCAdapter::s_eagerness = 0;


    // tiling count, base tiles in both dimensions, tiling domain excess, max splits, eagerness
    TC::BTCArguments btcArgs{
        3, // tiling count
        TC::Pair<int>(1,1), // base tiles in x and y dimensions
        0.1f, // padding of the tiles beyond sampling domain
        s_maxSplits, // maximum number of splits, default infinity
        s_eagerness // eagerness to split, default 0
        // transformation defaults to Spherical
    };

    void record(const DTreeRecord& rec, EDirectionalFilter directionalFilter,
                EBsdfSamplingFractionLoss bsdfSamplingFractionLoss) {
        // Match vanilla guided_path semantics exactly so spatial-tree refine()
...
public:
    GuidedPathTracer(const Properties &props) : MonteCarloIntegrator(props) {
        ...
        // BTC directional structure knobs (consumed by BTCAdapter::btcArgs).
        BTCAdapter::s_maxSplits = props.getInteger("maxSplits", 216);
        BTCAdapter::s_eagerness = props.getInteger("eagerness", 4);
